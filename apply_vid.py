...
def cleanup_temp_frames(folder):
    """Remove all temporary frame files and folders"""
    if os.path.exists(folder):
        shutil.rmtree(folder)
        print(f"Temporary folder {folder} removed.")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the input video for style transfer",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="./output",
        help="Output directory to save the stylized video",
    )
...
def get_video_framerate(video_path):
    """Retrieve the framerate of the input video using ffprobe"""
    cmd = [
        "ffprobe",
        "-v", "error",
        "-select_streams", "v:0",
    ...
    # Convert fraction framerate to float if necessary
    if '/' in framerate_str:
...
def extract_frames(video_path, output_folder):
    ...
    cmd = ["ffmpeg", "-i", video_path, os.path.join(output_folder, "frame_%05d.png")]
    subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    print(f"Frames extracted to: {output_folder}")


def process_frame(model, encoder, frame, output_path: str):
...
def main():
    ...
    # Extract framerate from the input video
    original_framerate = get_video_framerate(args["video_path"])
    print(f"Original video framerate: {original_framerate} FPS")
    ...
    output_video_path = os.path.join(args["output"], "stylized_video.mp4")
    combine_frames_into_video(output_frames_folder, output_video_path, frame_rate=original_framerate)

    cleanup_temp_frames(frames_folder)
    cleanup_temp_frames(output_frames_folder)
