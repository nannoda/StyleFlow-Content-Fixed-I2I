...
from model.trainers.Trainer_StyleFlow import (
    Trainer,
    set_random_seed,
    merge_model,
)  # ,Trainer_PCA
...
def parse_args():
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="../scripts/output/wikiart/model_save/187500.ckpt.pth.tar",
    )
...
def main():
    ...
    test_dataset = get_data_loader_folder_pair(
        args["rootA"],
        args["rootB"],
        args["infotxt"],
        args["batch_size"],
    ...
    for batch_id, (imgA, imgB, name) in enumerate(test_loader):
        ...
        output_name = os.path.join(
            args["output"], args["task_name"], "img_gen", name[0]
        )
